...
do
	...
	cvlc v4l2:///dev/video0:chroma=h264 :input-slave=alsa://hw:1,0 --sout '#transcode{acodec=mpga,ab=128,channels=2,samplerate=44100,threads=4,audio-sync=1}:duplicate{dst=standard{access=file,mux=mp4,dst="'$DT'.mp4"},dst=standard{access=http,mux=ts,mime=video/ts,dst=:8099}}' 
	echo ----------------------------------------------------------
done
